...
// SYNC: structurally identical to loki/probe.go — duplicated per
// the cross-module no-share rule documented in #542. Keep both
// copies in sync.
...
// "audit/webhook: startup verification …" and include the sanitised
...
func probeEndpoint(rawURL string, tlsCfg *tls.Config, cfg *Config) error {
	...
	if err != nil {
		return fmt.Errorf("audit/webhook: startup verification failed for %s: %w (set verify_on_startup: false to skip)",
...
// Returns an error wrapping "audit/webhook: startup verification: …"
// for any unparseable or unsupported-scheme URL.
func parseProbeURL(rawURL string) (host, addr, scheme string, err error) {
	u, err := url.Parse(rawURL)
	if err != nil {
		return "", "", "", fmt.Errorf("audit/webhook: startup verification: parse url: %w", err)
	}
	if u.Scheme == "" || u.Host == "" {
		return "", "", "", fmt.Errorf("audit/webhook: startup verification: url missing scheme or host")
	}
	host = u.Hostname()
	port := u.Port()
	if port == "" {
		...
		case "http":
			port = "80"
		default:
			return "", "", "", fmt.Errorf("audit/webhook: startup verification: unsupported scheme %q", u.Scheme)
		}
	}
...
// closes rawConn too). The clone matters: without it the runtime's
// pinned-roots policy can silently diverge from the probe's
// verification path (#286 security review).
func probeTLSHandshake(ctx context.Context, rawConn net.Conn, tlsCfg *tls.Config, host, rawURL string) error {
	...
	if err := tlsConn.HandshakeContext(ctx); err != nil {
		_ = rawConn.Close()
		return fmt.Errorf("audit/webhook: startup verification failed for %s: tls handshake: %w (set verify_on_startup: false to skip)",
			sanitizeURLForLog(rawURL), err)
	}
	_ = tlsConn.Close()
	return nil
...
// Single source of truth: any divergence between the two would be a
// regression (probe accepts a target the runtime rejects, or vice
// versa). Currently a one-knob mapping; extending the policy means
// updating exactly this helper.
